#!/usr/bin/env node
/**
 * STATUS CENSUS — `tools/tmp/rules_census.mjs` with three additions the status-lock and
 * AI-hazard work needed and could not get by any other route. A COPY rather than an edit,
 * because a measurement agent is live on the original and one owner per file is the
 * hardest constraint on this project.
 *
 * What is different, and why each one:
 *
 *  1. PER-ROLE SPLITS. The original aggregates player and enemy into one mean, so it can
 *     say "31.4% of engaged time is spent movement-locked" but not WHO IS LOCKED. Every
 *     balance conclusion about a lock rule turns on that split — a rule that removes a
 *     lock the player was WINNING costs the player win rate, and a rule that removes one
 *     the player was SUFFERING pays it back. Same for pot and fog damage, which the
 *     arena pass just showed land 100% on the enemy.
 *  2. `smart2`. The `smart` policy tests LINE OF SIGHT BEFORE RANGE, so across 1,080 wu
 *     of a 27-box map it sits in "strafe, no LOS" for the whole match and never closes;
 *     `smart2` is the identical policy with that one clause reordered. Both are
 *     reported: `smart` for comparability with every number on record, `smart2` for a
 *     player that actually engages. Both now come from
 *     `tools/tmp/scripted_player.mjs` — see the driver block below, and note that
 *     "lifted verbatim from `arena_probe.mjs`", which is what this line used to say,
 *     is the whole reason a fixed driver defect went on being measured for a day.
 *  3. DEFAULT ARENA is `tools/arena.gameplay.json` — the SHIPPED layout. The original
 *     defaults to `tools/tmp/arena.frozen.json`, which is untracked and is now the
 *     PRE-2026-08-05 layout (spawns on the centre line, dead pot). Every number below is
 *     against the shipped one unless `--arena` says otherwise.
 *
...
// ─────────────────────────────────────────────────────────────────────────────
// the hand on the controls — IMPORTED, not copied
// ─────────────────────────────────────────────────────────────────────────────
/**
 * 271 lines used to sit here, lifted from `tools/match-sim.mjs` and re-lifted into
 * `roster_table.mjs` and `pacing_ladder.mjs` afterwards. `match-sim.mjs` fixed a real
 * defect in that driver on 2026-08-05 — the stuck detector ran during the COUNTDOWN,
 * so every match began with up to 900 ms of latched sideways walking — and the fix
 * could not reach a verbatim copy. Five instruments, one stale driver.
 *
 * It now lives in `tools/tmp/scripted_player.mjs`, once. Both historical defects stay
 * reachable BY FLAG so any figure this tool has printed can still be re-derived:
 *
 *   --nav-countdown-bug         the latched sideways detour
 *   --decide-during-countdown   the decision loop draws seeded RNG before the whistle,
 *                               which makes the stream at the whistle a function of
 *                               COUNTDOWN LENGTH — a timing edit then re-seeds every
 *                               match and presents as a balance finding
 *
 * `node tools/tmp/driver_guard.mjs` asserts neither can come back.
 */
...
function runMatch(playerId, enemyId, policy, seed) {
  ...
  const m = {
    ...
    damageBySource: {},
    /** Damage TAKEN, split by victim. The aggregate cannot say who the pot is hurting. */
    damageByRoleSource: { player: {}, enemy: {} },
    ...
    for (const ev of evs) {
      switch (ev.type) {
        ...
        case 'hit-landed': {
          ...
          m.damageByRoleSource[ev.targetRole][k] = (m.damageByRoleSource[ev.targetRole][k] ?? 0) + ev.amount;
...
console.log(`║ driver scripted_player.mjs rev ${DRIVER_REV}${driver.isHistorical ? `  ⚠️ HISTORICAL: ${Object.entries(DRIVER_FLAGS).filter(([, v]) => v).map(([k]) => k).join(' + ')} — reproduction only, NOT a current number` : ''}`);
console.log(`╚══════════════════════════════════════════════════════════════════════════\n`);

const summary = {
  clockMs: MATCH_DURATION_MS, nMatches,
...
for (const policy of POLICIES) {
  ...
  // ── WHO IS LOCKED ── the split the aggregate above cannot show. A rule that removes a
  // lock the PLAYER was winning costs the player win rate; one that removes a lock the
  // player was suffering pays it back. Both are happening at once and only this separates
  // them. Shares are of ENGAGED time (the denominator that survives a layout change).
  const roleStat = (kind, role, field) => mean(rows.map((r) => r[kind][role][field]));
  const roleLong = (kind, role) => rows.map((r) => r[kind][role].longestMs);
  console.log(`\n     WHO IS LOCKED (share of ENGAGED time · longest unbroken · applications/match)`);
  for (const role of ['player', 'enemy']) {
    console.log(`       ${role.padEnd(7)} stun ${pct((roleStat('stun', role, 'ticks') * DT) / engMs).padStart(6)}  max ${s(vmax(roleLong('stun', role))).padStart(6)}  apps ${roleStat('stun', role, 'applications').toFixed(2).padStart(5)}` +
      `   ·   slow ${pct((roleStat('slow', role, 'ticks') * DT) / engMs).padStart(6)}  max ${s(vmax(roleLong('slow', role))).padStart(6)}  apps ${roleStat('slow', role, 'applications').toFixed(2).padStart(5)}`);
  }
  ...
  summary.policies[policy] = {
    ...
    /** Per-role splits — see the WHO IS LOCKED block above for why these exist. */
    byRole: Object.fromEntries(['player', 'enemy'].map((role) => [role, {
      stunMs: roleStat('stun', role, 'ticks') * DT,
      stunEngagedShare: (roleStat('stun', role, 'ticks') * DT) / engMs,
      stunApps: roleStat('stun', role, 'applications'),
      stunReapply: roleStat('stun', role, 'reapply'),
      stunLongestMax: vmax(roleLong('stun', role)),
      stunLongestP90: quant(roleLong('stun', role), 0.9),
      slowMs: roleStat('slow', role, 'ticks') * DT,
      slowEngagedShare: (roleStat('slow', role, 'ticks') * DT) / engMs,
      slowApps: roleStat('slow', role, 'applications'),
      slowReapply: roleStat('slow', role, 'reapply'),
      slowLongestMax: vmax(roleLong('slow', role)),
      slowLongestP90: quant(roleLong('slow', role), 0.9),
      dmgTaken: Object.fromEntries(['weapon', 'fog', 'hazard', 'trail'].map((k) =>
        [k, mean(rows.map((r) => r.damageByRoleSource[role][k] ?? 0))])),
    }])),
  };
  // ── WHO THE HAZARDS HURT ── 100% of pot damage landing on one side is a bug, not a
  // difficulty setting, and the aggregate share cannot see it.
  {
    const take = (role, k) => mean(rows.map((r) => r.damageByRoleSource[role][k] ?? 0));
    const sideShare = (k) => { const p = take('player', k), e = take('enemy', k); const t = p + e; return t > 0 ? `${pct(p / t)} player / ${pct(e / t)} enemy` : '— (none dealt)'; };
    console.log(`\n     WHO THE HAZARDS HURT   pot ${sideShare('hazard')}   ·   fog ${sideShare('fog')}   ·   trail ${sideShare('trail')}`);
    console.log(`       HP/match taken  player: pot ${take('player', 'hazard').toFixed(1)} fog ${take('player', 'fog').toFixed(1)}  ·  enemy: pot ${take('enemy', 'hazard').toFixed(1)} fog ${take('enemy', 'fog').toFixed(1)}`);
  }
