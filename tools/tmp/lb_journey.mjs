...
/**
 * How many times to walk the whole path in ONE page session.
 *
 * ⚠️ **TWO IS THE MINIMUM AND IT IS NOT ARBITRARY.** The first run went red on
 * *"the persisted profile is readable"* — and the product was fine: a fresh browser has
 * NEVER WRITTEN `food-arena.profile.v1`, so the before-read was `null` and the payout
 * delta had nothing to subtract from. Hardcoding the defaults here would be a copy of
 * `profile.ts` that goes stale; running the path twice makes trip 2's delta exact against
 * a baseline the product itself wrote. It is also the thing a single-trip gate cannot see
 * at all — what LEAKS between a match and the menus, which is `journey.mjs`'s whole
 * reason for existing.
 */
const TRIPS = Number(arg('trips', '2'));
...
try {
  for (let trip = 1; trip <= TRIPS; trip++) {
    ...
    // ⚠️ ABSENT IS LEGAL ON TRIP 1 AND ONLY ON TRIP 1. `profile.ts` writes
    // `food-arena.profile.v1` on the first CHANGE, not on boot, so a fresh browser has
    // nothing stored. From trip 2 the product has written its own baseline and an absent
    // key would mean the payout never persisted at all — which is the defect.
    ok(`home mounted; persisted profile ${trip === 1 ? 'may legitimately be absent (first boot)' : 'MUST exist by now'}`,
      trip === 1 || before !== null,
    ...
    if (before !== null) {
      deltasMeasured++;
      ok('the profile in localStorage MOVED — the card is not fiction', moved,
      ...
      // No baseline to subtract from, so the weaker claim: the product WROTE one, with a
      // banked result in it. The strong delta row runs on the next trip.
      ok('first boot: the payout created the persisted profile', after !== null && !!after.economy?.lastMatch,
        `coins →${after?.economy?.coins} · lastMatch ${JSON.stringify(after?.economy?.lastMatch ?? null)}`);
    }
  ...
  // 🚨 NON-VACUITY. Every payout row below is guarded by `before !== null`, and a run
  // where that never held would report a clean sweep having measured no payout at all —
  // `[].every()` in a different costume. At least one trip must have had a baseline.
  ok(`at least one trip measured a payout DELTA against a written baseline`,
    deltasMeasured > 0, `${deltasMeasured} of ${TRIPS} trips`);
} finally {
  await browser.close();
}
