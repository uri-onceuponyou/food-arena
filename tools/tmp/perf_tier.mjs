...
// perf_tier.mjs ONLY: extra query string appended to every scene URL, so a run can be
// pinned to a render tier (?tier=high|medium|low). Every scene URL already contains a
// '?', so this must start with '&'. THE ONLY INTENDED DIFFERENCE FROM `tools/perf.mjs`.
