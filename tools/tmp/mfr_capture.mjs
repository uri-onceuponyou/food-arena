#!/usr/bin/env node
/**
 * MFR_CAPTURE — a byte-faithful fork of `tools/tmp/q1_capture.mjs` with exactly TWO
 * changes, both of which exist to answer one question:
 *
 *   > If a MEDIAN-DENSITY frame still scores ~4.7, framing is not the constraint
 *   > and the tile field is.
 *
 * `867d1c0` measured that the frame six blind critics called *"an empty test floor"*
 * sits in the 0.7th percentile of its own map for cover-box count. That is a claim
 * about the CAPTURE RULE, not about the arena. This tool shoots the same match, the
 * same matchup, the same eligibility band, the same viewport and the same seeded
 * profile at a DIFFERENT anchor, so one critic can be spent on the difference.
 *
 * -- CHANGE 1: the anchor is overridable ------------------------------------
 * `q1_capture.mjs` derives one anchor (centre + 1.684 x potRadius on +x) and offers
 * no way past it -- correct for a round panel, useless for an experiment about the
 * anchor. `--px/--py` names one; with neither, the derived anchor is used unchanged,
 * so the default behaviour of this fork is the default behaviour of its parent.
 *
 * WARNING: `judgeAnchor()` still runs and still PRINTS every arm. `--force` is required
 * to shoot an anchor it refuses, and the refusal is copied into the run report, because
 * an experiment that quietly relaxes the rule it is testing is not an experiment.
 *
 * -- CHANGE 2: the report records WHERE THE PLAYER ACTUALLY WAS --------------
 * This is a defect in the parent, found while re-deriving `867d1c0`, and it is the
 * reason this change is not cosmetic.
 *
 * `q1_capture.mjs` records `dAnchor` and `dPot` per candidate but never the player's
 * position. The drive loop only pulls the player back when `dAnchor > 240`, so a
 * capture happens ANYWHERE inside a 240 wu leash. `shots/q1/cap/capture-report.json`
 * records `match_donut_taco_05.png` at dAnchor 157, dPot 299 -- so the frame that was
 * scored was NOT shot at (1560,1000), and every density number published for it was
 * measured at a point the camera was not looking from. Solving the two recorded
 * distances puts it at (1682,1098) or (1682,902): cover 3 / share 6.6% or cover 5 /
 * share 12.0%, against the published 2 / 9.9%.
 *
 * The conclusion survives -- every one of those is still a bottom-decile frame -- but
 * the numbers do not, and the fix is to write `px`/`py` down. They cost nothing and
 * they are the only way to measure the frame you actually shipped.
 *
 * Usage:
 *   node tools/tmp/mfr_capture.mjs --preflight --px 1200 --py 900
 *   node tools/tmp/sx_snap.mjs --root /private/tmp/fa-mfr -- \
 *     node tools/tmp/mfr_capture.mjs --url '{URL}' --px 1200 --py 900 \
 *       --out shots/mfr/a900 --seed-profile
 */
...
if (args.preflight || args['known-bad']) {
  ...
  // ── THE FORK'S OWN ARM: judge the anchor this run will actually shoot. The parent's
  // preflight reports the DERIVED anchor whatever `--px/--py` says, which would let an
  // experiment print a green preflight for a point it is not going to use.
  let requested = null;
  if (args.px !== undefined && args.py !== undefined) {
    requested = report(`REQUESTED ANCHOR --px ${args.px} --py ${args.py}`,
      { x: Number(args.px), y: Number(args.py) }, ARENA);
  }
  ...
  if (requested) {
    verdict.push([`the REQUESTED anchor (${args.px},${args.py}) passes every arm`, requested.ok]);
    if (!requested.ok) {
      verdict.push([`  ...and its ONLY failures are: ${requested.checks.filter((c) => !c.ok).map((c) => c.name).join('; ')}`, true]);
    }
  }
  console.log('\n── meta ──');
  for (const [name, ok] of verdict) console.log(`  ${ok ? 'ok  ' : 'FAIL'} - ${name}`);
  const allOk = verdict.every(([, ok]) => ok);
  console.log(`\n${allOk ? '✅ PASS' : '🔴 FAIL'}  mfr_capture preflight: ${verdict.filter(([, o]) => o).length}/${verdict.length}`);
...
/** The drive loop's pull-back radius. 240 is `q1_capture.mjs`'s literal; named here so the
 *  report can state how far a capture is allowed to wander from the anchor being tested. */
const LEASH = Number(args.leash ?? 240);

const ANCHOR = (args.px !== undefined && args.py !== undefined)
  ? { x: Number(args.px), y: Number(args.py) }
  : derivedAnchor();
const ANCHOR_SOURCE = (args.px !== undefined && args.py !== undefined) ? 'explicit --px/--py' : 'derived';
const ANCHOR_VERDICT = judgeAnchor(ANCHOR, ARENA);
{
  for (const c of ANCHOR_VERDICT.checks) console.log(`  ${c.ok ? 'ok  ' : 'FAIL'} - ${c.name}  [${c.detail}]`);
  if (!ANCHOR_VERDICT.ok && !args.force) {
    console.error('anchor refused - run --preflight, or pass --force and say so in the report');
    process.exit(3);
  }
  if (!ANCHOR_VERDICT.ok) {
    console.error(`\n!! FORCED: this anchor FAILS - ${ANCHOR_VERDICT.checks.filter((c) => !c.ok).map((c) => c.name).join('; ')}`);
    console.error('   The failure is recorded in capture-report.json and must be declared with any score.\n');
  }
  console.log(`anchor (${ANCHOR.x},${ANCHOR.y}) [${ANCHOR_SOURCE}], centre (${ARENA.center.x},${ARENA.center.y}), potR ${POT.radius}\n`);
}
  ...
  while (candidates.length < WANT && Date.now() - t0 < Number(args.budgetMs ?? 480_000)) {
    ...
    if (eligible) {
      ...
      const res = await captureSettled(page, {
        path: `${OUT}/${name}`, label: `match action ${n}`, tool: 'mfr_capture',
      ...
      candidates.push({
        ...
        painted: res.painted,
        stats: res.stats,
        // THE PARENT DOES NOT RECORD THESE, AND EVERY DENSITY NUMBER PUBLISHED FOR ITS
        // FRAMES WAS MEASURED AT THE NOMINAL ANCHOR INSTEAD. See this file's header.
        px: p.x,
        py: p.y,
        ex: e.x,
        ey: e.y,
