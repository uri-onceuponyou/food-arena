...
The five gates in `CLAUDE.md` were the whole story when there were five. There are now **120**, and
...
| `node tools/tmp/as_cost.mjs --selftest` | **32** | prices *"the AI has no search behaviour"* (`src/arena/types.ts:95`) **before** anyone writes one, by staging an ORACLE arm — `visible` forced true in `ai.ts` and nothing else — which is a hard **upper bound on what any search behaviour can buy**. 🚨 Its headline: at 1400×1000 the oracle moves the aggregate **+0.0 pp** and **2 of 110** paired matchups; at 2800×2000 it is **bit-identical, 0 of 110, on both policies**. ⚠️ And it **falsifies §48's own attribution**: on the arm §48 blamed on the searchless AI the belief was stale for **0 of 2,020,248 ticks** — there was no concealment on that map — while `match-sim`'s stall rule read **51.45%, longest 22.50 s**, all of it with a FRESH belief. That collapse was navigation, and `b9bc00e` fixed it (542/801 of 880 never-contacted → 26/45). Reproduces §48's published table to the digit on `git show b9bc00e~1`. Its own known-bads include a **cage the AI cannot escape** (which the per-tick "motionless" counter reads as 0.0% and match-sim's span rule as 20.7% — so the per-tick number is reported and never quoted as a stall) and an **unshielded `ax_layout` import**, which hijacks the process |
...
| `node tools/tmp/hl_sweep.mjs --selftest` | ⚠️ browser, **8** | the weapon-halo sweep and its 8 controls. ⚠️ **The selftest is AGGREGATE and needs the WHOLE sweep** — a weapon above the halo split exercises the "the picture changed" side and has nothing to assert on the null side, and one below it is the reverse — so `--chars`/`--weapon` make it *invalid*, not merely narrower. It is the slowest browser gate here, over ten minutes on a contended box |
| `node tools/tmp/sc2_manifest.mjs` | ⚠️ browser, **54** | 3 bases × 14 checks + 4 known-bad controls for the Add-to-Home-Screen manifest. Its controls are the good kind: a root-absolute icon 404s on all three bases, a root-absolute `start_url` lands outside the base, a de-based href 404s, and pure HEAD produces no link, no meta and no icon. 🔴 **3 of its 54 are RED today, and correctly**: that last control pins *pure HEAD has no manifest*, and `92e794a` LANDED the manifest — so the control now reads its own success as a failure. Same shape as the ×4 fixtures: a fixture pinned to a state that has since changed. Owner's call, not `gatecount`'s |
