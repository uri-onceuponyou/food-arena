...
The five gates in `CLAUDE.md` were the whole story when there were five. There are now **68**, and
...
| `node src/game/sim.test.mjs` | **323** | sim, combat, AI, navigation, status, concealment rules. ⚠️ Was 253 before §29c — attacking breaks the cover and reveals you (§26(j)–(m)) — and **287 before §27**, the N-fighter container: the slot-array invariants, the perception matrix's index and identity, `damagedMask` per-victim, the event protocol's slot ids, and the timeout tiebreak **rung by rung against the two-way rule it replaced** (rung 3 is reached by no corpus — 0 of 3520 forced-immortal timeouts) |
...
| `node tools/tmp/conceal_lab.mjs --selftest` | **39** | walk-through concealment: bit-identity, the band whose centre is legal and near edge is not, and the "no arena is failing this today" control. **Was undocumented until `gatecount` went looking**. ⚠️ **Was 22 until 2026-08-10**, when `--bitid` stopped being a STATE-ONLY test: it now compares the `GameEvent[]` `stepMatch` returns as well. §E proves that extension does something (a sim whose only difference is `death` before `hit-landed` is passed by the old harness and failed by this one); §G proves the CORPUS can be blind — at `LEVEL_MIN` both `damageMul`s are exactly 1.0, so a sim that resolves the WRONG attacker is invisible until `--levels 15:1`; §F guards `--ablate`'s mask and perturbations. New modes: `--bitid --corpus normal,timeout,countdown`, `--levels p:e`, `--ablate` |
