...
The five gates in `CLAUDE.md` were the whole story when there were five. There are now **111**, and
...
| `node tools/tmp/sp_place.mjs --selftest` | **22** | sweeps every cell of a 2 wu lattice against every spawn rule. 🔴 **Its answer is that the shipped 1400×1000 map seats FOUR, not six**: of 327,561 cells only **2,186** survive, in **two** mirror-pair regions — and three pairs need three. ⚠️ **The runway rule binds by 47×** (dropping it alone: 2,186 → 103,926), so it is the constraint to argue with, not the map size |
| `node tools/tmp/sp_gate.mjs --selftest` | **22** | 180° point symmetry asserted **on the browser dump**, not on the source. Fails on a 1 wu nudge of **either** member, an axis mirror, **two pairs' partners swapped** (still a symmetric *set* — the *pairing* breaks), an odd list, and a dump that has drifted 1 wu from `kitchen.ts`. Also pins `spawns.length === MAX_FIGHTERS` |
