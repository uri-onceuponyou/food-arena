...
| 🔴 **weapon cards are fiction** | Uri found it on Water Bottle. **17 faults across 10 blurbs** (`node tools/tmp/wm_gate.mjs`), down from 23/13. ⚠️ **The "20 of 34" this line carried was MINE and was never falsifiable** — the original audit had no closed vocabulary, so it counted similes and bare plurals as claims; the gate re-derives it as **13**, and *"9 of 11 characters" reproduced exactly*. Byte-identical to the 2D prototype — **and so is the mechanic set (10 slow / 5 stun both sides), so the promises were NEVER true in either codebase.** He chose BUILD over reword |
